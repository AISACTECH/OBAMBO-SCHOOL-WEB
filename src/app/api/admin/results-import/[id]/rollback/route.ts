...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isSafeInteger(importId) || importId < 1) {
    return NextResponse.json({ error: "Invalid import." }, { status: 400 });
  }

  const result = await db.transaction(async (tx) => {
    const [importRecord] = await tx
      .select({ id: resultImports.id, rolledBack: resultImports.rolledBack })
      .from(resultImports)
      .where(eq(resultImports.id, importId))
      .limit(1);
  ...
  if ("error" in result) {
    return NextResponse.json({ error: result.error }, { status: result.error === "Import not found." ? 404 : 409 });
  }
  ...
  await logAudit({
    ...
    targetId: importId,
    details: { recordsRemoved: result.recordsRemoved },
  });
