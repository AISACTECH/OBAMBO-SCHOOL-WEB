...
export async function GET() {
  ...
  const limit = rateLimit(`google-oauth:${session.id}`, 3, 10 * 60 * 1000);
  if (!limit.allowed) return NextResponse.json({ error: "Too many Google authorization attempts. Please try again later." }, { status: 429 });
